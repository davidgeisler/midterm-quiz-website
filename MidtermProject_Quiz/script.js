...
const quizAmbiance = document.getElementById('quiz-ambiance');
const hoverSfx = document.getElementById('hover-sfx');
const clickSfx = document.getElementById('click-sfx');
const resultSfx = document.getElementById('result-sfx');
...
document.addEventListener("click", (event) => {
    ...
    if (event.target.matches(".choices")) {
        ...
        resetHighlight();
        event.target.style.backgroundColor = "#1a82c3";
...
function loadChoiceState (user_choice) {
    ...
    if (user_choice) {
        ...
        buttonChoice.style.backgroundColor = "#1a82c3";
...
function loadNextQuestion () {
    ...
    if (currentQuestion) {
        ...
        if (currentQuestion.img_src && currentQuestion.img_src.trim() !== "") {
            elements.questionImg.setAttribute("src", currentQuestion.img_src);
            elements.questionImg.classList.remove("d-none");
        } else {
            elements.questionImg.setAttribute("src", "");
            elements.questionImg.classList.add("d-none");
        }
...
function updateSounds() {
    ...
    if (appState === MENU) {
      ...
      bgMusic.play();
      quizAmbiance.pause();
      quizAmbiance.currentTime = 0;
    } 
    else if (appState === QUIZ) {
      bgMusic.pause();
      bgMusic.currentTime = 0;
      quizAmbiance.volume = 0.3;
      quizAmbiance.play();
    } 
